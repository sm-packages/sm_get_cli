...
- **验证：** 运行 `dart analyze`、`dart test`、`dart pub publish --dry-run`；用 `dart pub global activate --source path .` 验证 `get`、`getx` executable 均能生成，但版本输出必须用 Git 安装或发布后的 hosted 安装验收，再检查 `get --version` 和 `getx --version` 均显示 `pubspec.yaml` 中的版本。同时搜索残留的运行时代码引用：`rg "package:get_cli|sm-packages/get_cli" bin lib pubspec.yaml`。
...
- **必须保持的不变量：** `-i`/`-o` 显式参数优先于配置，配置优先于默认路径；只读取输入目录中的 JSON；嵌套键稳定展平；非法 JSON 和非法 Dart 标识符必须失败；输出文件名和类名配置继续生效。
...
- **验证：** 在临时输出目录分别验证默认配置和 `-i`/`-o` 覆盖，检查自定义文件名、类名、嵌套键、引号和换行转义；再以非法 JSON 和非法键验证失败路径。当前没有自动回归覆盖。
...
8. 合并完成后，用新 merge commit 及其上游父提交更新“精确上游基线”，重新审计能力，并在同一提交同步代码和 `FORK.md`。
...
- 三份 README 仍引用已删除的 `samples_file/controller.dart.example`。这是已知文档缺陷，不是需保留的不变量；后续应改为现存模板路径并单独验证链接。
- 隔离的 path-source 全局激活可以生成 `get` 和 `getx`，但当前 `PubspecLock.getVersionCli` 通过 `Platform.script` 相邻路径寻找 lockfile，因而 path 安装的 `--version` 只报告“没有找到已安装版本”。Git 全局安装的两个命令目前均能显示 `1.9.3+3`；正式发布前应修复 path 安装的版本定位并增加安装形态回归测试。
