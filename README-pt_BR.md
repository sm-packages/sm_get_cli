...
| Prefixo configurável do pacote GetX | `get_package_prefix: sm_getx` gera `package:sm_getx/get.dart`; qualquer pacote GetX compatível pode ser selecionado, e o padrão continua sendo `get`. |
...
get_cli:
  ...
  use_state: true
  # O padrão é get. Use o nome do pacote do seu fork do GetX.
...
O valor padrão de `get_package_prefix` é `get`. Defina-o como o nome do pacote de qualquer fork compatível do GetX, como `sm_getx`, e os geradores Flutter internos junto com a variável `@{import}` dos templates personalizados gerarão `import 'package:sm_getx/get.dart';`. O valor deve ser um nome de pacote Dart válido em letras minúsculas; valores inválidos encerram o comando com um erro de configuração. O projeto já deve depender do pacote configurado; essa opção não adiciona, remove nem altera dependências. Projetos Get Server continuam gerando `package:get_server/get_server.dart`.
